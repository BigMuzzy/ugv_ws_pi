...
if [[ ! -e $FLAG_FILE ]]; then
  ...
  curl -sf https://install.transitiverobotics.com/files/package.json -o package.json
  curl -sf https://install.transitiverobotics.com/files/.npmrc -o .npmrc
  curl -sf https://install.transitiverobotics.com/files/.env -o .env

  echo "TR_USERID=zaslonov_alexander" >> .env
  echo "TR_INSTALL_HOST=https://install.transitiverobotics.com" >> .env
  echo "TR_ROBOT_TOKEN=6t3p3k6o425u4im532w385f" >> .token
